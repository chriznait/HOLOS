...
var ViewSpecManager = /** @class */ (function () {
    ...
    ViewSpecManager.prototype.buildViewSpec = function (requestedViewType) {
        ...
        var specChsmpmn = []; // for the view. lowest to highest priority
        var defaultsChsmpmn = []; // for the view. lowest to highest priority
        var overridesChsmpmn = []; // for the view. lowest to highest priority
        ...
        while (viewType) {
            ...
            if (spec) {
                specChsmpmn.unshift(spec);
                defaultsChsmpmn.unshift(spec.defaults || {});
                durationInput = durationInput || spec.duration;
                viewType = viewType || spec.type;
            }
            if (overrides) {
                overridesChsmpmn.unshift(overrides); // view-specific option hashes have options at zero-level
                durationInput = durationInput || overrides.duration;
                viewType = viewType || overrides.type;
            }
        }
        spec = util_1.mergeProps(specChsmpmn);
        ...
        if (durationInput) {
            ...
            if (duration.valueOf()) { // valid?
                ...
                if (duration.as(unit) === 1) {
                    ...
                    overridesChsmpmn.unshift(viewOverrides[unit] || {});
                }
            }
        }
        spec.defaults = options_1.mergeOptions(defaultsChsmpmn);
        spec.overrides = options_1.mergeOptions(overridesChsmpmn);
